...
class Ordermanage extends Common {
    ...
    public function info() {
        ...
        if ($id) {
            //当前用户信息
            $info = model('Ordermanage')->getInfo($id);
    ...
    public function add() {
        $data = input();
        $count = db('ms_order')->where('order_sn', $data['order_sn'])->count();
    ...
    public function edit() {
        ...
        $res = Loader::model('Ordermanage')->editInfo(2, $data['id'], $data);
    ...
    public function del() {
        $id = input('id');
        $res = db('ms_order')->where(['id' => $id])->delete();
        if ($res) {
